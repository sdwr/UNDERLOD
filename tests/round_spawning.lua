...
a, sm = arena_for(75, {spawn_director = {setpoints = {swarmer = 10}}})
...
-- Run the actual director for every current ordinary campaign level.
for _, level in ipairs({1, 2, 3, 4, 5, 7, 8, 9, 10}) do
  local budget = LEVEL_PACING[level].kill_quota
  a, sm = arena_for(budget, LEVEL_SPAWN_POOLS[level])
  ...
  assert(exhausted_power >= budget and exhausted_power < budget + 100)
end

-- A specials-only level uses the same finite budgeting and clear rules.
a, sm = arena_for(600, {spawn_director = {setpoints = {sniper = 2}}})
for frame = 1, 2000 do
  a.t:update(.05)
...
for _, path in ipairs({'spawns/spawnmanager.lua', 'spawns/levelmanager.lua',
  'enemies/enemy.lua', 'enemies/regular/splitter.lua', 'ui/progress_bar.lua',
  'level_classes/arena.lua', 'level_classes/combat_level.lua'}) do assert(loadfile(path)) end
print('PASS: finite budget, delayed spawns, no score-based wins, scripted events, scatter accounting, loss priority, debug, bosses, all campaign directors, swarmer-free levels, progress display, returning enemies, and splitter offspring.')
