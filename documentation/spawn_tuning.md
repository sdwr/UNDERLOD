...
2. **Specials queue** (`SpawnManager:tick_spawn_director`) — tanks, small
   ...
   whichever slot is furthest below its setpoint. Paced by **whole-field**
   power fill (swarmers included), so a packed swarm delays the next special.
   Held closed for the first `SPAWN_DIRECTOR_OPENING_GRACE` (7) seconds of
   the level.
...
Both lanes share the same machinery: per-slot setpoints, the ramp
(setpoints scale `ramp.from -> ramp.to` across kill-quota progress, default
0.8 -> 1.2), ceilings (`setpoint * SPAWN_DIRECTOR_CEILING_MULT`), pending
(in-flight) tracking, and weighted offscreen placement.
...
- Average clump size `G` is computed from `SWARMER_GROUP_MIX` (currently
  ~8.33: 1/3 chance of a 4-6 scatter group, 2/3 chance of a clustered 8-12).
...
Resulting steady-state intervals with defaults (FILL_TIME 8, G 8.33):

| level | swarmer setpoint | ramped (x0.8 open) | I_base open | I_base level end (x1.2) |
|---|---|---|---|---|
| L1 | 22 | 17.6 | 5.6s | 3.7s |
| L2 | 30 | 24.0 | 4.1s | 2.7s |
| L3 | 40 | 32.0 | 3.1s | 2.1s |
| L4-10 | 48 | 38.4 | 2.6s | 1.7s |

Caveat: at L1 a single clump is nearly half the field, so "80% in 8s"
quantizes down to ~5.4s; the fill-time promise really governs L3+ where
multiple fires matter. If L1 should open lazier, shrink clump sizes at low
setpoints (group mix), don't raise FILL_TIME.
...
| front-load the level | `ramp = {from = 1.3, to = 0.9}` | opens heavy, eases off; also speeds the opening cadence since I_base uses the ramped setpoint |
...
| more overshoot room | `ceilings`, `ceiling_mult` | per-slot or global ceiling override |
...
| `SPAWN_DIRECTOR_RAMP_FROM/TO` | 0.8 / 1.2 | default within-level escalation (per-level `ramp` overrides) |
