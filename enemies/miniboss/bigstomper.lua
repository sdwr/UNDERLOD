  ...
  --set attacks
    self.t:cooldown(attack_speeds['ultra-slow'], function() local target = self:get_closest_target(self.attack_sensor, main.current.friendlies); return target end, function()
      ...
      if closest_enemy then
        Stomp{group = main.current.main, unit = self, team = "enemy", x = self.x, y = self.y, rs = 100, color = red[0], damage = function() return self.dmg end, chargeTime = 2.5, level = self.level, parent = self}
      end
    end, nil, nil, 'attack')
end
